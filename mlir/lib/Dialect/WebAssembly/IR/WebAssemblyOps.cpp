...
//===----------------------------------------------------------------------===//
// ExtendFromLowBytesSOp
//===----------------------------------------------------------------------===//

ParseResult ExtendFromLowBytesSOp::parse(::mlir::OpAsmParser &parser,
  ...
  result.addAttribute(
      ExtendFromLowBytesSOp::getBitsToTakeAttrName(OperationName{
          ExtendFromLowBytesSOp::getOperationName(), parser.getContext()}),
      parser.getBuilder().getI64IntegerAttr(nBits));
  result.addTypes(inType);
  return success();
}

void ExtendFromLowBytesSOp::print(OpAsmPrinter &p) {
  p << " " << getBitsToTake().getUInt() << " low bits from ";
  p.printOperand(getInput());
  p << ": " << getInput().getType();
}

LogicalResult ExtendFromLowBytesSOp::verify() {
