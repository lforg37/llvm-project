...
namespace mlir {
#define GEN_PASS_DEF_WASMSSAMLIRTOEMBEDDER
...
struct TrapOpConversion : WasmRuntimeOpConversion<TrapOp> {
  using WasmRuntimeOpConversion<TrapOp>::WasmRuntimeOpConversion;
  LogicalResult
  matchAndRewrite(TrapOp trapOp, typename TrapOp::Adaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    rewriter.replaceOpWithNewOp<func::CallOp>(trapOp, this->runtimeSymbols.trap,
...
struct WasmSSAMLIRToEmbedderPass
    : public impl::WasmSSAMLIRToEmbedderBase<WasmSSAMLIRToEmbedderPass> {
...
std::unique_ptr<Pass> mlir::createWasmSSAMLIRToEmbedderPass() {
  return std::make_unique<WasmSSAMLIRToEmbedderPass>();
}
