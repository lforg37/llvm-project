...
struct WasmEncodings {
  ...
  static constexpr uint8_t endByte{0x0B};
};
...
private:
  ...
  inline parsed_inst_t parseSpecificInstruction(OpBuilder &builder, Value operand) {
    return emitError(getLocation(), "Unknown instruction opcode: ")
  ...
  ///
  /// @tparam curBit is the first bit for which the pattern is not fixed
  ///
  /// @tparam highBits is the fixed pattern that this instance handles for the
  /// 8-curBit bits
  template <uint8_t curBit=0, uint8_t highBits=0>
  inline parsed_inst_t dispatchToInstParser(uint8_t opCode, OpBuilder &builder,
                                            Value operand) {
    static_assert(curBit <= 8,
                  "Fixing more than 8 bits on a uint8_t doesn't make sense");
    if constexpr (curBit < 8) {
      constexpr uint8_t bitSelect{1 << (7 - curBit)};
      constexpr uint8_t nextHighBitStem = highBits << 1;
      constexpr uint8_t nextCurBit = curBit + 1;
      if (opCode & bitSelect)
        return dispatchToInstParser < nextCurBit,
               nextHighBitStem | 1 > (opCode, builder, operand);

      return dispatchToInstParser<nextCurBit, nextHighBitStem>(opCode, builder,
                                                               operand);

    } else {
      return parseSpecificInstruction<highBits>(builder, operand);
    }
  }
